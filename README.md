...
Для изменений данных итемов предпочитать MCP (`handbook_item_create/update` и
связанные инструменты). SQL-миграции нужны для изменения схемы и формата данных;
обычные правки описаний и механик существующих записей не требуют новой миграции.

У эффектов список `application_sources` ведётся вручную в редакторе. При добавлении
или изменении применения нужно актуализировать этот список у эффекта.
Для эффекта, создаваемого заклинанием, используем тот же `icon_image_id` (либо
`icon_svg_id`), что у заклинания: общий ID, без копирования файла и без механизма
наследования. Эффект после завершения другого эффекта использует ту же иконку.
