...
ssh -i "$SSH_KEY" "$VM_USER@$VM_HOST" "bash -s -- '$BUILD_COMMIT'" <<'REMOTE'
  set -e
  expected_commit="$1"
  chmod +x ~/dndshare.new ~/session-image-sync.new ~/race-image-sync.new ~/class-image-sync.new ~/spell-rune-sync.new ~/item-cover-sync.new ~/bestiary-image-sync.new ~/dndshare-run.sh ~/fetch-secrets.sh
