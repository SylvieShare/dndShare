...
func main() {
	...
	for index, image := range itemcovers.Catalog {
		stored, err := uploadImage(ctx, objects, image)
		if err != nil {
			log.Fatal(err)
		}
		linked, err := database.UpsertSystemItemCover(ctx, image.ObjectKey, stored.URL, image.FileName, image.MimeType, image.Size, image.TypeID, image.Aliases)
		if err != nil {
			log.Fatalf("register %s: %v", image.Key, err)
		}
		if linked == 0 {
			log.Fatalf("register %s: no item matched aliases %v", image.Key, image.Aliases)
		}
		log.Printf("synced %d/%d: %s, linked items: %d", index+1, len(itemcovers.Catalog), image.Key, linked)
	}

	retiredKeys, err := database.ListRetiredItemCoverKeys(ctx)
	if err != nil {
		log.Fatalf("list retired item covers: %v", err)
	}
	for _, key := range retiredKeys {
		if err := objects.DeleteObject(ctx, key); err != nil {
			log.Fatalf("delete retired item cover %q: %v", key, err)
		}
		log.Printf("deleted retired item cover: %s", key)
	}
}

func uploadImage(ctx context.Context, objects *storage.Service, image itemcovers.Image) (storage.StoredObject, error) {
	data, err := itemcovers.Read(image)
	if err != nil {
	...
	stored, err := objects.UploadItemCover(ctx, bytes.NewReader(data), image.Size, image.ObjectKey, image.MimeType)
	if err != nil {
