...
func main() {
	...
	for index, image := range classimages.Catalog {
		stored, err := uploadImage(ctx, objects, image)
		if err != nil {
			log.Fatal(err)
		}
		linked, err := database.UpsertSystemClassImage(ctx, image.ObjectKey, stored.URL, image.FileName, image.MimeType, image.Size, image.Aliases)
		if err != nil {
			log.Fatalf("register %s: %v", image.Key, err)
		}
		if linked == 0 {
			log.Fatalf("register %s: no class item matched aliases %v", image.Key, image.Aliases)
		}
		log.Printf("synced %d/%d: %s, linked items: %d", index+1, len(classimages.Catalog), image.Key, linked)
	}
}

func uploadImage(ctx context.Context, objects *storage.Service, image classimages.Image) (storage.StoredObject, error) {
	data, err := classimages.Read(image)
	if err != nil {
