...
import spellType from './spell-type.json'
const params = new URLSearchParams(location.search)
const app = createApp({ setup() {
  const state = reactive({ version: '2024', race: params.has('unselected') ? null : fixture.races.find(r => params.has('gnome2014') ? r.id === 4032 : r.name === (params.has('human') ? 'Человек' : params.has('tiefling') ? 'Тифлинг' : params.has('dragonborn') ? 'Драконорождённый' : params.has('goliath') ? 'Голиаф' : params.has('elf') ? 'Эльф' : params.has('aasimar') ? 'Аасимар' : 'Гном')), subrace: null, raceVariant: null, asiChoice: [], raceSkillIds: [], featIds: [], raceLangIds: [] })
  ...
  provide('createWizard', { state, grants, races: ref(fixture.races), subraces: computed(() => (fixture.subraces || []).filter(s => s.data.race === state.race?.id)), loading: ref(false), raceAbilities: ref(fixture.abilities),
    ...
    featPool: ref([]), featLimit: ref(1), toggleFeat: () => {}, setFeatSelection: () => {}, featEligibility: () => true, featComplete: ref(true), raceFeatureChoices: ref([]),
  })
...
useItemTypesStore(pinia).types = [{ id: 3, name: 'Расовые способности', fields: [{ key: 'desc', name: 'Описание', type: 'description' }] }, spellType, { id: 7, name: 'Черты', fields: [{ key: 'category', name: 'Категория', type: 'select', filter: true, options: [{ value: 'origin', label: 'Черта происхождения' }, { value: 'general', label: 'Универсальная черта' }] }] }]
