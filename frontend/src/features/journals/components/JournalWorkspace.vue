<template>
  <section class="journal-workspace" :class="{ 'journal-workspace--session': sessionUuid }">
    ...
    <template v-else>
      <header class="journal-cover">
        ...
        <div class="journal-cover-copy">
          ...
          <p>{{ journal ? `Разделов: ${sections.length} · Записей: ${eventCount}` : 'Личная летопись или общая история вашей группы' }}</p>
        </div>
      ...
      <template v-if="journal">
        ...
        <DndDiarySessionCard v-if="selectedSection" :key="`${journal.uuid}:${selectedSection.id}`"
          ...
          @remove-event="removingEvent = $event" @editing="setEditing" @dragging="setDragging"
    ...
    <DndDiarySessionModal v-if="sectionDraft" :session="sectionDraft" :mode="creatingSection ? 'create' : 'edit'" :busy="busy"
    ...
    <ConfirmDialog v-if="removingSection" title="Удалить раздел?" :loading="busy"
    ...
    <ConfirmDialog v-if="removingEvent" title="Удалить событие?" :loading="busy"
...
const locked = computed(() => busy.value || Boolean(editingId.value))
