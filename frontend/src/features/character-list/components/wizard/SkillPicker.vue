<template>
  <div class="step">
    ...
    <p v-if="!options.length && emptyText" class="hint">{{ emptyText }}</p>
    ...
    <div class="list">
      <div
        ...
        :key="opt.id"
        class="skill"
        :class="{ on: isSelected(opt.id), off: !isSelected(opt.id) && atLimit }"
...
const props = defineProps({
  title: { type: String, default: 'Владение навыками' },
...
const isSelected = (id) => props.selected.some((selectedId) => String(selectedId) === String(id))
function toggle(id) {
  if (!isSelected(id) && atLimit.value) return
  emit('toggle', id)
}
