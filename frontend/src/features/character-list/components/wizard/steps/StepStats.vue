<template>
  <div class="step">
    <div class="stats-bar">
      ...
      <button class="qb" title="Раскидать стандартный набор по классу" @click="quickBuild">
      ...
      <span v-if="state.statMethod === 'pointbuy'" class="budget" :class="{ over: pointsLeft < 0 }">Очки {{ pointsLeft }} / {{ BUDGET }}</span>
    </div>

    <p class="hint">{{ hint }}</p>

    <div v-if="state.statMethod === 'roll' && !state.rollPool.length" class="roll-cta">
      <button class="roll-btn" @click="rollStats">
      ...
      <div v-for="s in STATS" :key="s" class="stat" :class="{ primary: primaryAbilities.includes(s) }">
        ...
        <div class="stat-ctl">
          <template v-if="state.statMethod === 'pointbuy'">
            ...
            <button class="step-btn" :disabled="(state.scores[s] ?? 8) >= 15 || pointsLeft <= costStep(s)" @click="bump(s, 1)">+</button>
          </template>
...
.qb svg { width: 15px; height: 15px; }
.qb:hover { color: var(--text-1); }
.budget { font-size: 12px; font-weight: 600; color: var(--text-2); margin-left: auto; }
