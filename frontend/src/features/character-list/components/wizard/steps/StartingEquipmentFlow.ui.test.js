...
describe('starting equipment wizard flow', () => {
  ...
  it('uses taller 64px weapon rows with stacked facts on the class step', () => {
    ...
    expect(itemReference).toMatch(/\.item-reference-economy \{[^}]*flex-direction: column;/)
    expect(itemReference).toContain('white-space: normal')
    expect(itemReference).toContain('class="item-reference-economy"')
    expect(itemReference).toContain('.item-reference-economy > span + span::before { content: none; }')
