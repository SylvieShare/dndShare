...
describe('ViewSession participant rail', () => {
  ...
  it('uses the chapter canvas as the full workspace with a right tool rail', () => {
    ...
    expect(styles).toContain('--chapter-safe-left: calc(var(--participant-rail-width) + 42px);')
  ...
  it('moves the player rail through compact, normal and combat states without losing the saved preference', () => {
    ...
    expect(styles).toMatch(/\.workspace-dock--left\s*\{[^}]*width:\s*var\(--participant-rail-width\);/s)
    expect(styles).toMatch(/\.campaign-workspace--combat \.workspace-dock--left\s*\{[^}]*width:\s*calc\(var\(--participant-rail-width\) \+ var\(--participant-selection-extra\)\);/s)
  })
  ...
  it('reuses one encounter in the combat workspace and the expanding player rail', () => {
    ...
    expect(styles).toContain('width: calc(var(--participant-rail-width) + var(--participant-selection-extra));')
    expect(styles).toContain('--chapter-safe-left: calc(var(--participant-rail-width) + var(--participant-selection-extra) + 42px);')
  ...
  it('rolls a configured challenge for selected combatants in embedded row results', () => {
    ...
    expect(encounterChallengeSource).toContain("encounter.value.combatants.filter(combatant => combatant.position !== 'dead' && selectedUids.value.has(combatant.uid))")
    ...
    expect(encounterRowSource).toContain('min-height: 74px;')
  ...
  it('keeps combat controls in stable labelled groups', () => {
    expect(encounterSource).toContain('class="enc-action-group-label">Состав сцены</span>')
    expect(encounterSource).toContain('class="enc-action-group-label">Выбранные</span>')
    expect(encounterSource).toContain('v-if="props.isDm" class="enc-action-group" aria-label="Состав сцены"')
  ...
  it('opens row actions from the tile and keeps concrete controls independent', () => {
    ...
    expect(encounterSource).toContain(':actor-name="enc.npcName(enc.detailNpc)"')
    expect(encounterComposableSource).toContain('npcActor: c => encounterEventData(c, npcData.npcName(c)).npcActor')
