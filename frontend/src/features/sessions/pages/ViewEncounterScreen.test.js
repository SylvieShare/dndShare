...
describe('public encounter portraits', () => {
  ...
  it('keeps emphasized colored creature letters by active names and on queue portrait corners', () => {
    ...
    expect(initiativeStyles).toContain('background: color-mix(in srgb, var(--screen-combatant-color) 22%, var(--scrim));')
    expect(initiativeStyles).toContain('color: color-mix(in srgb, var(--screen-combatant-color) 72%, var(--text-on-accent));')
    expect(initiativeStyles).toContain('box-shadow: 0 0 20px color-mix(in srgb, var(--screen-combatant-color) 38%, transparent);')
...
describe('public encounter composition', () => {
  it('keeps the active turn below a cyclic stacked queue', () => {
  ...
  it('places a tightly packed square queue above the larger active card without cropping icons', () => {
    ...
    expect(initiativeStyles).toContain('width: var(--spotlight-width);')
    ...
    expect(initiativeStyles).toContain('translate: calc(var(--queue-stack-offset) * 18px) calc(var(--queue-stack-offset) * 4px);')
    ...
    expect(pageSource).toContain("'--queue-stack-offset': Math.min(index - queueStackStart.value, 4)")
  ...
  it('scales the full combat stage and recalculates queue capacity from its logical width', () => {
    ...
    expect(pageSource).toContain('screenCombatLayout(viewport.value, presentation.value?.displayScale, turnQueue.value.length)')
