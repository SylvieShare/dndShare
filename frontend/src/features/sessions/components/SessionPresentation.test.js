...
describe('session presentation workspace', () => {
  it('keeps materials as a primary DM workspace but removes them from the display dropdown', () => {
    ...
    expect(control).toContain("label: 'Бой'")
    expect(control).toContain("label: 'Картинка'")
    expect(control).toContain("label: 'Письмо'")
    expect(control).toContain("label: 'Ничего'")
    expect(control).toContain('presentation.removeMaterial')
    expect(control).toContain('Убрать')
    expect(control).not.toContain('Очистить')
    expect(control).not.toContain('Остановить')
    expect(control).toContain('Эффект на экране игроков')
    expect(control).toContain('icon: CloudRain')
    expect(control).toContain('icon: CloudFog')
    expect(control).toContain('icon: Flame')
    expect(control).toContain('icon: Snowflake')
    expect(control).toContain('icon: CloudLightning')
    expect(control).toContain('Настройки трансляции')
    expect(control).toContain('settingsOpen')
    ...
    expect(control).toContain('Масштаб боевой трансляции')
    expect(control).toContain('type="range" min="75" max="125" step="5"')
    expect(presentationState).toContain('setDisplayScale')
  ...
  it('uses one public screen for blackout, full-screen materials and combat', () => {
    ...
    expect(presentationState).toContain("const removeMaterial = () => save({ mode: 'idle', visible: true")
  })
  ...
  it('keeps live connected-screen state on the trigger without replacing the mode summary', () => {
    ...
    expect(control).not.toContain('Нет подключённых экранов')
    expect(control).not.toContain('Получают обновления в реальном времени')
    expect(control).toContain('class="presentation-mode"')
  ...
  it('uses reusable relation editing for material contexts and keeps screen launch in the header control', () => {
		expect(materialEditor).toContain('<UniversalRelationList')
		expect(materialEditor).toContain(':source-type="type"')
