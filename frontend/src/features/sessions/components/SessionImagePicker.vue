<template>
  <div class="session-image-field">
    <div class="session-image-current">
      <img v-if="currentUrl" :src="currentUrl" :alt="currentLabel" :style="customSelected ? customPreviewStyle : null" />
    ...
    <AppModalFrame v-if="pickerOpen" extra-wide title="Выбрать изображение" @close="pickerOpen = false">
      <div class="session-image-browser">
        <nav class="session-image-category-nav" aria-label="Категории изображений">
          <button v-for="category in categories" :key="category.key" type="button" @click="scrollTo(category.key)">
            {{ category.label }}
          </button>
        </nav>
        ...
        <div class="session-image-sections">
          <section
            v-for="category in categories"
            :key="category.key"
            :ref="element => rememberSection(category.key, element)"
            class="session-image-section"
          >
            ...
            <div class="session-image-grid" role="radiogroup" :aria-label="category.label">
              ...
              >
                <img :src="presetUrl(preset.key)" :alt="preset.label" />
...
const props = defineProps({
  ...
  catalog: { type: String, default: 'session' },
...
const categories = computed(() => props.catalog === 'npc' ? NPC_IMAGE_CATEGORIES : SESSION_IMAGE_CATEGORIES)
const presetUrl = key => props.catalog === 'npc' ? npcImagePresetUrl(key) : sessionImagePresetUrl(key)
const currentPreset = computed(() => categories.value.flatMap(category => category.presets).find(preset => preset.key === props.modelValue))
const currentUrl = computed(() => props.customSelected ? props.customPreview : presetUrl(props.modelValue))
const currentLabel = computed(() => props.customSelected ? 'Своё изображение' : (currentPreset.value?.label || 'Не выбрано'))
...
function selectPreset(key) { emit('select', key); pickerOpen.value = false }
...
.session-image-current > img { width: 126px; height: 86px; display: block; border-radius: 8px; object-fit: cover; }
...
@media (max-width: 640px) { .session-image-current { grid-template-columns: 92px minmax(0, 1fr); }.session-image-current > img { width: 92px; }.session-image-current > button { grid-column: 1 / -1; justify-content: center; }.session-image-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
</style>
