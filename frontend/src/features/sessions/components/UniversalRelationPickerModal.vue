<template>
  <AppModalFrame :title="modalTitle" @close="$emit('close')">
    <div v-if="!fixedType" class="entity-picker-types">
    ...
    <div v-if="groups.length" class="entity-picker-results">
      <section v-for="group in groups" :key="group.key">
        <div class="entity-picker-group-head">
          <h3>{{ group.label }}<span>{{ group.items.length }}</span></h3>
          <button
            v-if="creatableTypeKeys.has(group.key)"
            type="button"
            class="entity-picker-create"
            :aria-label="`Создать: ${group.singular.toLowerCase()}`"
            @click="$emit('create', group.key)"
          ><span>Создать</span><Plus :size="14" /></button>
        </div>
...
const query = ref('')
const activeType = ref(props.fixedType || 'all')
const filters = [{ key: 'all', label: 'Все' }, ...SESSION_ENTITY_TYPES]
const fixedTypeMeta = computed(() => SESSION_ENTITY_TYPES.find(type => type.key === props.fixedType) || null)
const modalTitle = computed(() => fixedTypeMeta.value ? `Выбрать: ${fixedTypeMeta.value.singular.toLowerCase()}` : 'Добавить связь')
const creatableTypeKeys = computed(() => new Set(props.creatableTypes))
const groups = computed(() => {
  ...
  return SESSION_ENTITY_TYPES
    .filter(type => (props.fixedType ? props.fixedType === type.key : activeType.value === 'all' || activeType.value === type.key))
    ...
    .filter(group => group.items.length || creatableTypeKeys.value.has(group.key))
})
</script>
