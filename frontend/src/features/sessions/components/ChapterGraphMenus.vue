    ...
    <div v-if="activeChapter" class="chapter-action-menu">
      ...
      <RowActionSubmenu label="Статус главы" :min-width="230">
        <template #trigger="{ open }">
          <button type="button" class="chapter-submenu-trigger" :aria-expanded="open">
            Изменить статус
            <span class="chapter-action-chevron" :class="{ 'chapter-action-chevron--open': open }">›</span>
          </button>
        </template>
      ...
      <RowActionSubmenu v-if="arcs.length > 1" label="Переместить в арку" :min-width="220">
        <template #trigger="{ open }">
          <button type="button" class="chapter-submenu-trigger" :aria-expanded="open">
            Переместить в арку
            <span class="chapter-action-chevron" :class="{ 'chapter-action-chevron--open': open }">›</span>
...
.chapter-action-menu button:hover,
.chapter-submenu-trigger:hover { background: var(--surface-raised); color: var(--text-1); }
...
.chapter-submenu-trigger { display: flex; align-items: center; justify-content: space-between; }
.chapter-action-chevron { transition: transform 120ms ease; }
.chapter-action-chevron--open { transform: rotate(90deg); }
