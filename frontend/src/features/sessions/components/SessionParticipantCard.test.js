...
describe('SessionParticipantCard actions', () => {
  ...
  it('opens a shared row action menu from the participant BaseTile', () => {
    ...
    expect(source).toContain('--participant-inset-shadow: inset 0 0 22px color-mix(in srgb, var(--participant-color) 42%, transparent);')
    expect(source).toContain('.p-card.base-tile--interactive:hover')
    expect(source).toContain('.p-card--current.base-tile--interactive:hover')
    expect(source).toContain('var(--participant-inset-shadow),')
