<template>
  <article class="event-row" :class="{ 'event-row--standalone': !grouped }">
    <SessionEventIcon v-if="!grouped" :event="event" />
    <div class="event-content">
      <div class="event-heading">
        <span>{{ sessionEventAction(event, entityName) }}</span>
...
import { sessionEventAction, sessionEventDetails } from '../lib/sessionEventEntity'
...
.event-heading { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; color: var(--text-2); font-size: 12px; line-height: 1.4; overflow-wrap: anywhere; }
.event-heading time { flex: none; color: var(--text-muted); font-size: 10px; font-variant-numeric: tabular-nums; }
