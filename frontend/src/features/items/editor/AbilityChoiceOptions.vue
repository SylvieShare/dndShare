<template>
  <div class="ability-action-fields">
    <BaseTile v-for="(option, index) in modelValue" :key="rowKey(option)" class="ability-rule-row">
      ...
      <AbilityRuleFields :fields="fields.filter(f => !['label', 'value', 'desc'].includes(f.key))" :data="option" @update:data="v => Object.assign(option, v)" />
    </BaseTile>
...
const props = defineProps({ modelValue: { type: Array, default: () => [] }, fields: { type: Array, default: () => [] } })
