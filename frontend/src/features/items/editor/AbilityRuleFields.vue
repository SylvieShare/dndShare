<template>
  <div class="ability-rule-fields">
    <AbilityRuleField v-for="field in basicFields" :key="field.key" :field="field" :model-value="data[field.key]" @update:model-value="value => update(field.key, value)" />
    <details v-if="advancedFields.length" class="ability-advanced" :open="hasAdvancedValues || undefined">
      ...
      <div class="ability-rule-fields">
        <AbilityRuleField v-for="field in advancedFields" :key="field.key" :field="field" :model-value="data[field.key]" @update:model-value="value => update(field.key, value)" />
      </div>
    </details>
  </div>
</template>
...
const props = defineProps({ fields: { type: Array, default: () => [] }, data: { type: Object, default: () => ({}) }, advanced: Boolean })
