...
export function catalogueFieldVisible(field, data, typeId, path, root = data) {
  ...
  if (typeId === 5 && /^(damage|rolls)\.save_(effect|condition|dc)$/.test(path)) return !!data.save_ability
...
export function updateCatalogueValue(data, field, value, path, typeId) {
  ...
  if (/^(damage|rolls)\.save_ability$/.test(path) && !value) { delete next.save_effect; delete next.save_condition; delete next.save_dc }
