...
const classGroups = [
  ...
  group('equipment', 'Стартовое снаряжение', 'starting_equipment starting_kit starting_gold', true),
]
export const catalogueProfiles = {
  19: { title: 'Магический предмет', primary: ['desc', 'type', 'rarity', 'attunement', 'attunement_requirement', 'activation'], groups: [group('economy', 'Цена, вес и использование', 'cost weight consumable recharge_note available_in_starting_shop treasure'), group('storage', 'Хранение и содержимое', 'is_container contents equipment_category measurement unit_cost_copper unit_weight'), group('weapon', 'Оружие', 'weapon'), group('armor', 'Доспех или щит', 'armor_base')] },
  1: { title: 'Оружие', primary: ['notes'], groups: [group('weapon', 'Вид и свойства', 'is_military is_long_range tags required_weapon_proficiencies mastery'), group('damage', 'Урон и дистанция', 'attacks universe_attacks range_min range_max'), economy] },
  ...
  5: { title: 'Заклинание', primary: ['lvl', 'schoolId', 'description', 'classes'], groups: [group('casting', 'Сотворение', 'time range duration concentration ritual components'), group('targets', 'Цели применения', 'application_targets', true), group('damage', 'Урон', 'damage', true), group('healing', 'Лечение', 'heal', true), group('effects', 'Накладываемые эффекты', 'status_effects', true), group('creation', 'Создание предметов', 'item_creation', true)] },
  ...
  10: { title: 'Зелье', primary: ['desc'], groups: [group('potion', 'Свойства зелья', 'rarity color'), group('use', 'Применение', 'usable', true), economy] },
  11: { title: 'Предыстория', primary: ['description', 'feature', 'feature_desc'], groups: [group('grants', 'Владения и языки', 'skills languages lang_choice tool_prof ability_options origin_feat_id'), group('equipment', 'Стартовое снаряжение', 'tool_items equipment_items starting_coins starting_gold', true), group('choices', 'Снаряжение на выбор', 'item_choices', true)] },
  ...
  15: { title: 'Эффект', primary: ['code', 'desc', 'thesis'], groups: [group('sources', 'Источники применения', 'application_sources'), group('effect', 'Применение эффекта', 'polarity color stacking duration concentration'), group('rules', 'Изменения показателей', 'derived_effects', true), group('actions', 'Действия эффекта', 'repeat_save ongoing_damage weapon_target weapon_damage on_end_effect end_on', true), group('defenses', 'Защиты', 'defenses', true), group('levels', 'Ступени эффекта', 'level max_level', true)] },
...
export function catalogueProfile(typeId, fields) {
  ...
  const baseGroups = profile.groups.some(group => group.keys.includes('usable')) ? [...profile.groups] : [...profile.groups, group('use', 'Применение', 'usable', true)]
  baseGroups.push(group('creation_sources', 'Создаётся магией', 'creation_sources', true))
  const groups = baseGroups.map(g => ({ ...g, fields: pick(g.keys) })).filter(g => g.fields.length)
