...
const sectionStyles = readFileSync(fileURLToPath(import.meta.resolve('@sylvieshare/share-ui/styles.css')), 'utf8')
...
describe('enemy detail section hierarchy', () => {
  it('gives bestiary sections semantic icons and a stronger combat accent', () => {
    ...
    expect(enemySource).toContain('label="Действия" tone="danger"')
  })

  it('renders illustrated headings with a framed icon and fading rule', () => {
    expect(enemySource).toContain("import { DetailSection } from '@sylvieshare/share-ui'")
    expect(sectionStyles).toContain('.detail-section-icon')
    expect(sectionStyles).toContain('.detail-section-rule')
    expect(sectionStyles).toContain('.detail-section--danger')
    expect(sectionStyles).toContain('linear-gradient(90deg,')
  })
