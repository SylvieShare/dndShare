    ...
    <template v-for="sub in (field.fields || [])" :key="sub.key">
      <div v-if="editor.isNestedFieldVisible(field.key, sub)" class="iem-sub-field">
        ...
        <select
          ...
          class="iem-select"
          :value="editor.objectValue(field.key)[sub.key] ?? ''"
          @change="editor.setObjectField(field.key, sub.key, $event.target.value || null)"
        >
          <option value="">—</option>
              ...
              <template v-for="nested in (sub.fields || [])" :key="nested.key">
                <div v-if="editor.isObjectArrayFieldVisible(sub, row, nested)" class="iem-sub-field">
                  ...
                  <select
                    ...
                    class="iem-select"
                    :value="row[nested.key] ?? ''"
                    @change="editor.setNestedObjectArrayField(field.key, sub.key, rowIndex, nested.key, $event.target.value || null)"
                  >
                    <option value="">—</option>
                    <option v-for="die in SYSTEM_DICE" :key="die.id" :value="die.id">{{ die.value }}</option>
                  </select>
