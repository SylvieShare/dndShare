<template>
  <RowActionSubmenu v-if="ctx.ownerMode && ctx.topSession && ctx.itemTransfers && !entry?.params?.magic?.lost" :label="`${purpose === 'use' ? 'На кого использовать' : 'Кому предложить'}: ${name}`" :min-width="260" :disabled="disabled || controller.state.busy">
    <template #trigger="{ open }">
      <RowActionItem :icon="purpose === 'use' ? Pill : Send" submenu :submenu-open="open" :disabled="disabled || controller.state.busy" @click="!open && controller.loadPlayers()">{{ purpose === 'use' ? 'Использовать на…' : 'Передать другому игроку' }}</RowActionItem>
    </template>
...
import { Pill, Send } from '@lucide/vue'
...
const props = defineProps({ purpose: { type: String, default: 'transfer' }, disabled: Boolean, source: { type: String, required: true }, entry: { type: Object, required: true }, name: { type: String, default: 'Предмет' } })
const emit = defineEmits(['close'])
const ctx = inject('charCtx', {})
const controller = computed(() => ctx.itemTransfers)
async function send(player, close) {
  if (!props.disabled && await controller.value.send(props.source, props.entry, player.charUuid, props.purpose)) { close(); emit('close') }
}
</script>
<style scoped>
