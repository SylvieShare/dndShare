...
export const hasMagicInstanceOptions = item => !!item?.data?.initial_charges || !!item?.data?.manual_size || actionableItemChoices(item).length > 0
...
export function fillMissingMagicBases(values, item, uid, selectedParams) {
  ...
  const params = item.data?.initial_charges ? initializeItemCharges(entry.params, selectedParams.magic?.max_use) : { ...entry.params }
