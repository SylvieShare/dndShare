...
const spell = { id: 500, data: { lvl: 0, range: { kind: 'ranged', distance: 120 }, damage: { dices: [{ dice_id: 'd10', count: 1 }], scaling: 'cantrip', instances: 1, addon_instances: 1 } } }
it('adds Charisma once per beam, permits negative modifiers, and leaves unrelated spells unchanged', () => {
  for (const [score, expected] of [[16,3],[8,-1]]) {
    ...
    expect(calc.spellRangeOverride(spell)).toBe('300 футов')
