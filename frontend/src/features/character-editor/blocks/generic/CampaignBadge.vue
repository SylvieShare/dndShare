  ...
  <BaseTile v-if="top && ctx.itemTransfers" framed class="campaign-block" data-tutorial="character-session">
    ...
    <div class="campaign-actions">
      <div class="campaign-action">
        <ActionButton variant="secondary" class="campaign-icon" aria-label="Игроки" :title="`Игроки: ${playerCount ?? 'загрузка'}`" @click="ctx.itemTransfers.open('players')">
          <template #icon><Users :size="24" aria-hidden="true" /></template>
        </ActionButton>
        <span class="campaign-count" aria-hidden="true">{{ playerCount ?? '—' }}</span>
      </div>
      <div class="campaign-action">
        <ActionButton variant="secondary" class="campaign-icon" aria-label="События" :title="`События: ${eventCount}. Входящих: ${ctx.itemTransfers.incomingCount}`" @click="ctx.itemTransfers.open('events')">
          <template #icon><Bell :size="24" aria-hidden="true" /></template>
        </ActionButton>
...
<style scoped>
.campaign-block { display: flex; flex-wrap: wrap; align-items: center; gap: 16px; padding: 18px; }
.campaign-heading { display: flex; flex: 1 1 120px; flex-direction: column; gap: 5px; min-width: 0; }
...
.campaign-action { position: relative; }
.campaign-icon { width: 48px; height: 48px; padding: 0; gap: 0; }
