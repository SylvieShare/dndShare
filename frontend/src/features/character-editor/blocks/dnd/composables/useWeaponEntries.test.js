...
it('saves and unequips an inventory attack without a duplicate or lost charges', async () => {
  const props = reactive({ block: { id: 'weapon' }, value: [{ uid: 'normal', item_id: 49 }], values: { items: { equipped: [{ uid: 'magic', item_id: 100, count: 1, params: { magic: { attuned: true, remaining: 4 } } }], sections: [] } } })
  ...
  expect(props.values.items.sections[0].items[0]).toMatchObject({ uid: 'magic', count: 1, params: { magic: { remaining: 1 } } })
  scope.stop()
})
