<template>
  <span v-if="ctx.maxSlotLevel > baseLevel && hasInlineSpellScaling(rule)" class="spell-scaling-formula" :style="{ '--scaling-size': `${compactSize}px` }" :title="hint" :aria-label="hint" role="img">
