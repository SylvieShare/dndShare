    ...
    <template #default="{ close }">
      <SpellCastControls :entry="entry" :cast-level="castLevel" :spend-by-default="ctx.availableSpellSlotOptions(entry).length > 0" application>
        <template #default="cast">
          ...
          <RowActionItem v-else :icon="UserRound" :disabled="cast.disabled" @click="apply(cast, ['self'], 0, close)">На себя</RowActionItem>
          <template v-if="othersAllowed">
            ...
            <RowActionItem v-else :icon="Crown" :disabled="cast.disabled" @click="apply(cast, [], 1, close)">Мастер — выберет цель</RowActionItem>
            ...
            <template v-for="player in controller?.recipients || []" :key="player.charUuid">
              ...
              <RowActionItem v-else :disabled="cast.disabled" @click="apply(cast, [player.charUuid], 0, close)">
          ...
          <ActionButton v-if="maximum(cast) > 1" :disabled="cast.disabled || selected.length + dmCount < 1 || selected.length + dmCount > maximum(cast)" @click="apply(cast, selected, dmCount, close)">Применить · {{ selected.length + dmCount }} / {{ maximum(cast) }}</ActionButton>
        </template>
      </SpellCastControls>
    </template>
  </RowActionSubmenu>
...
import { ActionButton, FormNumberInput, LoadingIndicator, RowActionSubmenu } from '@sylvieshare/share-ui'
...
const applicable = computed(() => links.value.length || props.entry.item?.data?.heal?.dices?.length)
...
const othersAllowed = computed(() => ctx.charCtx.topSession && controller.value?.state.settings?.interactions?.spells !== false)
watch(othersAllowed, allowed => { if (!allowed) { selected.value = selected.value.filter(key => key === 'self'); dmCount.value = 0 } })
const maximum = cast => Math.min(50, Math.max(1, Number(props.entry.item.data.application_targets?.count) || 1) + Math.max(0, Number(props.entry.item.data.application_targets?.per_slot) || 0) * Math.max(0, cast.castLevel - (Number(props.entry.item.data.lvl) || 0)))
const blocked = (key, cast) => cast.disabled || (!selected.value.includes(key) && selected.value.length + dmCount.value >= maximum(cast))
