<template>
  <footer v-if="source || audit.length" class="diary-meta" @click.stop @keydown.esc.stop="hide">
    <button v-if="source" type="button" :aria-label="source"
      @mouseenter="show('source', $event)" @mouseleave="hide" @focus="show('source', $event)" @blur="hide" @click="show('source', $event)">
      <Link2 :size="15" />
    </button>
...
import { Clock3, Link2 } from '@lucide/vue'
...
.diary-meta button { display: grid; place-items: center; width: 28px; height: 28px; padding: 0; border: 0; border-radius: 7px; background: transparent; color: var(--text-muted); cursor: help; }
