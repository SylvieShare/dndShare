<template>
  ...
  <template v-else>
    <div class="dawn-resources">
      ...
      <p v-if="!candidates.length && !done">Нет ресурсов для восстановления на рассвете.</p>
    ...
    <ActionButton v-if="!done" :disabled="!candidates.length || applying" @click="apply">Встретить рассвет</ActionButton>
...
function apply() {
  ...
  results.value = result.results; done.value = true; applying.value = false
  emit('apply', result)
}
load()
