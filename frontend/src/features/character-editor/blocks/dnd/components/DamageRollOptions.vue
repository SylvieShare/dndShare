  ...
  <template v-if="canAttack">
    <RowActionSubmenu v-for="scope in ['attack', 'damage']" :key="scope" :min-width="280">
      ...
      <template #default="{ close }">
        <WeaponRollControls :scope="scope" :preview="previewFormula" :options="menuOptions(scope)" v-model:critical="critical" v-model:two-handed="twoHanded" :versatile="versatile" :thrown="thrown" @select="select" @amount="setAmount" @roll="close(); emit(scope === 'attack' ? 'attack' : 'roll', options)" />
      </template>
    </RowActionSubmenu>
  </template>
...
const props = defineProps({ actions: { type: Array, default: () => [] }, versatile: Boolean, canAttack: Boolean, preview: Function })
const emit = defineEmits(['roll', 'attack'])
const critical = ref(false), twoHanded = ref(false), selected = ref([]), amounts = ref({})
