...
- **Feat** `feat_choice = { count }` → `grants.featChoice`; picks from handbook feats (**type 7**, loaded into `featPool`) → `state.featIds` → the sheet's `values.abilities_feats = [{id}]` block. Gifted Human.
