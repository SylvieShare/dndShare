...
**Race choices live on the Race step** (`StepRace`, under Происхождение) — two kinds, both read from the race/subrace item `data`:
...
- **Named variants** `variants: [{ value, label, desc, asi?, asi_choice? }]` → `grants.raceVariants`; a "pick one" whose chosen option's `asi`/`asi_choice` merge into grants (via `extractGrants(..., raceVariant)`). Human has **no subraces** — instead two variants: **Стандартный** (`asi` +1 to all six) and **Одарённый** (`asi_choice {count:2,bonus:1}` → reveals the floating picker). `state.raceVariant` is stored to the character as `values.race_variant`; `raceVariantsComplete` + `asiChoiceComplete` gate the Race step.

Both feed `finalScores` and `buildCharacterData({ raceVariant, asiChoice })`. The `asi_choice` field is in the type-8 schema seed (`variants` is data-only); `ON CONFLICT DO NOTHING` → fresh DBs only, prod carries the data via MCP.
