...
The shell mirrors the character-page aesthetic: every region is a **tile** (`.tile` = `background: var(--block-bg); border-radius: var(--r-lg)`, **no borders**) floating on the `--bg-deep` page, separated by `gap: 12px`. The page is **fixed-width**: `.layout` is `width: min(1480px, 100%); margin: 0 auto; padding: 12px` so it centers with `--bg-deep` gutters on wide screens. Do not reintroduce the old `border-right` dividers — separation is the gap between tiles.
...
- **Left-column button styling:** all of these are **borderless with a backing** (`--surface-1` default / `--surface-2` hover; `.pick-btn` uses an `--accent` tint; `.action-btn--danger` uses a `--danger` tint). The bottom **invite block** (`.invite-section`: create-character button + invite code) sits on a recessed `var(--bg-deep)` panel (`--r-md`), and the invite-code row uses `--block-bg` + `--border` to read against it.
