...
Карандаши редактируют содержимое на месте, существующий тип неизменяем.
...
и времени/автора — внизу справа. Polling приостановлен во время правки и drag.
...
иконки-маркеры. Заголовок каждой группы показывает компактный аватар субъекта:
у персонажа используется отдельная character icon с fallback на портрет листа,
у существа из бестиария — его icon/SVG/cover, а у старых или упрощённых существ
без связанного item — монограмма имени. Системные действия мастера получают
отдельный тематический `DM`-аватар с короной. Имя субъекта берётся из сохранённого `actorName`: для игрока это
