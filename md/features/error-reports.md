...
- `error_reports_list(limit?, offset?, summaryOnly?)` — read actionable `OPEN` approved reports with title and complete metadata. `summaryOnly=true` returns only `{hasReports}` and is intended for the pre-lock `limit=1` probe, so report payloads do not enter model context. Finished reports, unanswered AI questions, and serious changes awaiting ADMIN approval are omitted.
...
3. Keep the returned opaque `leaseId` for the duration of the run. The MCP trace necessarily contains tool inputs and outputs, so use the handle only as a direct MCP argument: never copy it into commentary, final answers, files, shell commands, memory, or subagent briefs. The default lease lifetime is 45 minutes; accepted values are 5–120 minutes. Repeat `error_reports_list(limit=500)` after acquisition, then immediately call `error_reports_claim(ids, leaseId)` with that complete snapshot before loading repository context.
