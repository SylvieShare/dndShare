...
- The wide form requires a short title and detailed description. It uses `html-to-image` to render both the selected element crop and the visible viewport. The element image is cropped from a render of the page tree rather than cloned from the element alone, so backgrounds, gradients, images, and other visual backing supplied by ancestors remain visible. Each JPEG preview occupies its own row and uses the full available form width, capped by the image's intrinsic width, before the form sends it with the current `window.location.href` and element JSON through `features/error-report/api/errorReportApi.js`. The crop has **Меньше/Больше** controls: users can move from the exact element to as many as three parent containers, then move back. Wider context is captured without adding a visual marker; the exact target remains available through the original `selector`, while `screenshotContextLevel`/`screenshotContextSelector` identify the captured container. Each capture is best-effort and independent.
