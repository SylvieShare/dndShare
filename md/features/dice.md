...
To wire a new caller: `useDiceStore().roll(title, expression, opts)` for parsed expressions, or `useDiceStore().pushEntry({ title, result, outcome?, color?, duration? })` for prebuilt results. No props/events — the popup is mounted once in `App.vue` and reads store state.
