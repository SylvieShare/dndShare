...
- `shared/lib/systemDice.js` — the fixed d4/d6/d8/d10/d12/d20/d100 catalogue. Its persisted ids are the unambiguous strings `"d4"`…`"d100"`; dice are rules-level values and are not loaded from `suggest`. All catalogue dice use the former d8 blue (`#7ab8e8`) as their default background/halo color.
- `shared/ui/SystemDie.vue` — shared animated die rendered with the exact geometry, dark glass, colored halo/liquid, highlight and monospace typography of the former suggestion SVGs. It accepts `sides`, an optional arbitrary displayed `value` (defaults to the maximum face), `size`, `color` and `animated`; the value scales to fit and only animated bubbles rise inside the clipped body like spell-slot bubbles (there are no static bubble dots).
