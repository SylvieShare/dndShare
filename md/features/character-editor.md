...
Принятие окончательно расходует дозу, не добавляя её в инвентарь получателя.
Перед принятием явно указано, что лечение и эффекты нужно отметить вручную.
...
события, принятие/отказ по передачам не вызывают всплывашек. Входящая передача
