...
Чипы справа снизу показывают число других участников и незавершённых передач, включая
...
Окно предлагает других персонажей той же сессии и явно показывает количество:
передаётся весь выбранный экземпляр/стопка. Предмет резервируется на сервере
...
При открытом собственном листе игрок получает уведомления только о предложениях
предметов своим персонажам (статус `pending`). Свои события, действия мастера
и других игроков, принятие/отказ не вызывают всплывашек. Входящая передача
предлагает «Открыть события» для принятия или отказа.
SSE-invalidation журнала обновляет и список передач, и историю; первая загрузка
не показывает старые события как новые. Собственный бросок не дублируется
...
без отдельных плиток. Каждая строка показывает иконку и имя отправителя,
иконку и имя предмета, состояние ожидания и доступные действия. Нажатие на
