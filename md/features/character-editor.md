...
В меню зелья самостоятельный пункт «Использовать на себя» применяет лечение,
...
При подключённой сессии рядом доступно «Использовать на…»: подменю показывает
...
«Принять применение» или отказаться; отправитель — «Отменить применение».
...
отправитель до резервирования, если в справочнике задан `consumption.choices`.
...
или не возвращают дозу дважды. Без сессии и в режиме чтения подменю скрыто.
...
Пункт «Передать другому игроку» доступен в меню вещей, оружия и зелий.
...
В поповере «Другие игроки» нажатие на строку открывает меню «Чат» / «Камень /
ножницы / бумага». Оба пункта открывают одно окно общения с выбранным персонажем;
режим переключается внутри. Чат хранит переписку в сессии, позволяет отправлять
...
передачами. Из входящего события можно сразу перейти к переписке или ответу.

В игре отправитель выбирает ход при вызове; получатель отвечает, не видя его.
После ответа окно и хроника показывают оба хода и победителя либо ничью. Вызов
...
Карточки передачи в поповере используют `BaseTile framed`, отдельные строки отправителя и предмета, превью применения и отделённые кнопки ответа. В меню зелий под пополнением доступно «Удалить (−1)» без применения механики. Подробности — [применение зелий](potion-automation.md).
