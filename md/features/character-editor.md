...
дублирующего заголовка; кнопка броска остаётся внизу. Превью и бросок используют `prepareDamageRoll`, включая
...
Именованные `weapon_uses` выбираются в подменю атаки оружия. Выбор показывает
дистанцию/расход и не тратит заряд; бросок сохраняет оплату и отдельные шаги
применения на UID. Под оружием и в инвентаре отображается общий блок применения:
формулы кубиками, условия тезисами, спасбросок/половина, отдельное подтверждение
попадания или промаха. Крит относится только к урону попадания. Незавершённые
шаги переживают перезагрузку и перемещение предмета, повторно ресурс не списывают.
Обычные атаки остаются доступны. Владелец управляет шагами; читатель наблюдает.
Метательное копьё молнии (284) разделяет урон линии и конечной цели в таком блоке.
Цели обучения не перемещены; desktop/mobile и роли покрыты прежними сценариями.
