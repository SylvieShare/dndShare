...
Spell item schema (`resources/items/item_5_shema.json`) includes optional structured `damage` and `heal` blocks, plus a top-level `source_kind` enum. Sync the DB `item_type` (id 5) after editing the JSON via `resources/items/update_item_5_shema.sql`.

- **`source_kind`** (`"base"|"addon"|"third"`): источник заклинания — `base` = книга правил (PHB), `addon` = офиц. дополнение (Tasha/Xanathar и т.п.), `third` = сторонний контент. Frontend enum: `SPELL_SOURCE_KIND`/`SPELL_SOURCE_KIND_LABELS` in `blocks/dnd/lib/spellEntry.js`. `filter: true`.
...
This data is populated by a **subagent enrichment pass** (not a Kotlin job): each spell's `nameEn` is its ttg.club slug — agents `POST https://5e14.ttg.club/api/v1/spells/<slug>`, parse base dice from `description`, the per-level increment from `upper`, and `source_kind` from `source.group`/`source.shortName`, then merge into the item via the `handbook_item_update` MCP tool. **Always pass `nameEn` on `handbook_item_update`** — omitting it nulls the `name_en` column (the ttg match key). Branching/random-type/hit-dice spells are left for manual review (only `source_kind` set).
