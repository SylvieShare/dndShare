...
### Применение зелья к себе

`POST /api/char/{uuid}/potion-use` принимает `{entryUid,clientActionId,version,optionKey?}`.
Доступен владельцу, включая персонажа без сессии. Сервер выбирает одну дозу из
сохранённого `values.potions`, строит план из справочника и одной транзакцией
...
смена экземпляра или варианта при том же UUID — 409. Неверная механика/невыбранный
