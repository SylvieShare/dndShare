...
- `session_material` — типизированный материал с областью `session`, `chapter`
  или `scene`. `image`, `video` и `map` ссылаются через `asset_id` на общий S3
  ...
  и корректную комбинацию `chapter_id/scene_id`.
