...
Дополнительные таблицы не создаются. Оружейная проекция экземпляра хранит свои
параметры в уже существующем JSON инвентаря; см. [магические предметы](features/magic-items.md).
...
Выбор экземпляра хранится в `params.armor_base_item_id`, оружейный — в
`params.weapon_base_item_id`. Новых таблиц нет. Обычные доспехи типа 12 продолжают
использовать собственную формулу `data.armor`.

### Миграция 89: явное включение виртуального оружия

`89_explicit_inventory_weapons.sql` обновляет подсказку оружейной механики типа 19.
Опциональный `entry.params.weapon_enabled` включает виртуальную атаку экземпляра
в `items.equipped`; отсутствие отметки означает выключенное состояние. Данные
персонажей миграция не переписывает, новые таблицы и копии в `values.weapon` не
создаются. Историческое автоматическое отображение экипировки прекращается;
пользователь включает нужные экземпляры через меню инвентаря.
