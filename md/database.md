...
хранится в `author_user_id`, а `actor_char_id` задаётся только для игрока,
действующего своим подключённым персонажем. `visibility` различает общие и
DM-only записи; `(session_id,client_action_id)` защищает повторную отправку от
дубликатов. Типизированные детали события находятся в `data`, а выдача идёт по
монотонному курсору `id`.
