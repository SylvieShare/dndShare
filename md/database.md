...
`parent_id`; дочерние подклассы не затрагиваются. Ручной legacy-синхронизатор сверяет
размер и SHA-256 встроенных JPEG перед загрузкой в S3 и повторно закрепляет связь
через `item.icon_image_id`.
