...
перемещает уже принадлежащие персонажам экземпляры из инвентаря в
`values.tools`. Владение при этом остаётся независимым значением в
`values.proficiencies['Инструменты']`.
...
зелья и инструменты используют `{uid,item_id,count,params,override}`, оружие —
