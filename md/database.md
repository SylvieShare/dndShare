...
- `session_scene_location`, `session_npc_location` и `session_npc_scene` —
  ...
  nullable `note`. `session_npc_relation` хранит одну неориентированную связь
  между двумя NPC в каноническом порядке `left_npc_id < right_npc_id` и её
  заметку. Все link rows удаляются каскадно вместе с любой стороной. Отдельной
...
подключённого персонажа, мастер — любого участника сессии. Роль автора (`gm` или
`player`) вычисляется при чтении по владельцу сессии. `visibility` различает
...
- `session_scene` — узел второго уровня с собственными `position_x/y` и
  ...
  nullable presentation preset ссылается на материал и музыкальный трек и
  хранит громкость, crossfade, эффект и способ появления;
  `session_scene_edge` соединяет только сценарии
  этой главы. Старые строки получают нейтральную обложку `discovery`;
- `session_scene_item` — текстовый, диалоговый, боевой блок, награда или
  изображение третьего уровня. Блок изображения хранит ссылку `material_id`,
  а не копию файла.
...
- `session_material` — типизированный материал. `image`, `video` и `map`
  ...
  Несколько необязательных связей с подписями находятся в
  `session_material_chapter` и `session_material_scene`; отсутствие связей
  означает доступность во всей сессии. Startup migration переносит прежнюю
  одиночную область в эти таблицы и удаляет `scope/chapter_id/scene_id`.
- `session_presentation_state` — единственное live-состояние экрана игроков на
  сессию: режим, видимость, материал/сценарий, эффект, переход и монотонная
  ревизия для polling-клиента.
