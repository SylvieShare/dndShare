...
- `journal` принадлежит либо одному `owner_user_id`, либо одной `session_id`;
  CHECK запрещает оба источника сразу, а unique по `session_id` оставляет у
  кампании ровно один общий дневник;
- `character_journal` хранит выбранный персонажем источник. Владелец может
  переключиться между своими личными дневниками и дневником сессии, в которой
  участвует этот персонаж;
