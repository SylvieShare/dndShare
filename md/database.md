...
- `session_entity_relation` хранит одну неориентированную связь между любыми
  двумя локациями, NPC, материалами, заданиями или сценариями. Пара `(type,id)` приводится
  ...
  удаление сущности очищает обе стороны. Старые специализированные связи NPC,
  локаций и материалов со сценариями мигрируются сюда и удаляются вместе со
  связями материалов к главам. Отдельной
...
- `session_material` — типизированный материал. `image`, `video` и `map`
  ...
  Контекст показа определяется универсальными связями материала со сценариями:
  без них материал доступен во всей сессии. Связей материала с главами в
  актуальной модели нет. Startup migration переносит прежние связи со
  сценариями и удаляет старые таблицы и `scope/chapter_id/scene_id`.
