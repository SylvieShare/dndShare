...
- `session_chapter` — узел, принадлежащий сессии и арке. Текстовый `number`
  уникален внутри арки. Здесь же хранятся описание, lifecycle-статус, один
...
- `session.current_chapter_id` — единственный текущий узел всей кампании. Сцены
  ссылаются на главу;
- `session_scene` — узел второго уровня с собственными `position_x/y`,
  принадлежащий одной главе; `session_scene_edge` соединяет только сценарии
  этой главы;
- `session_scene_item` — текстовый или списочный блок третьего уровня. Помимо
  содержимого и цвета он хранит координаты на холсте сценария;
