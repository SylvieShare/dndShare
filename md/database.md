...
- `journal_section` задаёт упорядоченный раздел летописи с названием и
  ...
  или участников боя. Тип существующей записи неизменяем. `position` остаётся
  стабильным порядком добавления для перечисления записей; последовательность
  истории теперь определяется связями, а не расположением карточек;
