...
- `journal_section` задаёт упорядоченный раздел летописи с названием и
  ...
  запись (`battle/dialog/event/newday`), HTML-описание и JSON payload диалога
  или участников боя. Тип существующей записи неизменяем. Порядок внутри
  раздела сохраняется транзакционно по полному списку ID; несоответствие текущему
  набору записей отклоняется. Unique `(section_id, position)` отложен до конца
  транзакции перестановки, чтобы обмен занятыми позициями был допустим;
...
Исторические авторы правок не восстанавливаются догадками (NULL). Перестановка
обновляет только позиции и время дневника, не время/автора изменения содержимого.
