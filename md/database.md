...
с его `{roll,bonus,total,revision?}` и удаляется целиком при сбросе результатов.
Опциональная `revision` обновляется при дополнительном броске с преимуществом
или помехой, даже если выбранное значение d20 не изменилось.
