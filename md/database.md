...
`svg_storage`, а `item.svg_id` указывает на иконку предмета. Последний startup
section `schema/06_item_icons.sql` идемпотентно создаёт отдельные SVG-строки для
существующих базовых оружия и обычного снаряжения PHB 2014, только если у item
ещё нет иконки; заданные вручную и пользовательские данные он не заменяет.
...
- `session.current_chapter_id` — единственный текущий узел всей кампании. Сцены
  по-прежнему ссылаются на главу.
...
1. Добавить финальное DDL в логический файл `internal/store/schema/*.sql`:
   foundation, handbook, characters, sessions, seed или item-icons. Порядок файлов задаёт
