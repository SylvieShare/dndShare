...
справочные записи предысторий. Каждая встроенная
...
экземпляра, которые хранятся на ссылке владения, а не в `item.data`. Инвентарь и
зелья используют `{uid,item_id,count,params,override}`, оружие — `item_id` и тот
же `params` рядом со своими боевыми настройками. Startup section
