...
- `session_quest` — упорядоченный журнал заданий с названием, описанием и
  lifecycle-статусом `planned/active/completed/failed`;
- `session_scene_location` и `session_npc_scene` хранят сюжетный контекст
  локаций и NPC отдельно от предметных связей;
- `session_entity_relation` хранит одну неориентированную связь между любыми
  ...
  удаление сущности очищает обе стороны. Старые `session_npc_location` и
  `session_npc_relation` мигрируются и удаляются. Отдельной
