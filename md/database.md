...
дописываются; запись `item_transfer` обновляет статус при принятии/отказе. Автор
...
с событием. Решение блокирует запрос и листы, переносит снимок в целевой лист,
...
Обычные события хроники дописываются; `item_transfer` — изменяемая запись статуса.
