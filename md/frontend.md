...
а на ширине до 768 px входящая и исходящая страницы кратко перекрываются: исходящая
страница исключается из потока, поэтому между фазами нет пустого кадра и разная
высота экранов не вызывает скачок раскладки. При `prefers-reduced-motion: reduce`
