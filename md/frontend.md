...
так что позиции следующих иконок не меняются из-за скрытия заголовка. Выбор
состояния rail сохраняется в local storage внутри общего компонента. Локальный
...
заголовка. Над поиском расположен `shared/ui/GameContextSelector`: в раскрытом
