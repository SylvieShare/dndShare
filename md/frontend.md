...
| Borderless tile / card chrome | `shared/ui/BaseTile` | re-styling `--surface` card wrappers; props `color`, `strip` (left color strip), `tint` (faint fill), `framed` (gradient + colored border, e.g. HP), `interactive` |
...
- **Form fields:** `shared/ui/form/`
  ...
  - `FormTextInput` — text input using `--surface-raised`, `--border-strong` and `--accent`. `v-model:value`. Forwards focus/blur/keydown via fallthrough; emits `enter` on Enter.
  - `FormTextarea` — sibling of FormTextInput for multiline.
  - `FormNumberInput` — spinner with `−`/`+` buttons. Like every field, it uses `--surface-raised` plus a `--border-strong` inset border so it remains legible on the shared `--bg` form canvas. `FormTextInput`, `FormTextarea`, `ValueSelect` and inline editor controls use the same surface model.
...
**Backdrop blur:** the window does **not** use `backdrop-filter` — it's unsupported/hardware-gated in some Chromium builds (Yandex Browser strikes it through) and our CSS minifier drops the unprefixed property. Instead the overlay dims with a tint derived from `--scrim`, and `setBgBlur()` applies a real `filter: blur(8px)` to `#app` while the window is open (toggled on open / close / unmount). `filter` is universally supported and blurs the actual content; the teleported overlay/panel sit outside `#app`, so they stay sharp.
