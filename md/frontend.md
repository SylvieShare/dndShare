...
| Popover/контекстное меню | `shared/ui/BasePopover.vue`, `RowActionMenu.vue`, `RowActionItem.vue` | Неблокирующий контент, привязанный к управляющему элементу. `RowActionMenu` поддерживает стандартный trigger с многоточием и feature-trigger через slot, остаётся внутри visual viewport, при нехватке места открывается вверх и ограничивает высоту внутренним скроллом. Каждый кликабельный пункт строится через `RowActionItem`: `action` выбирает стандартную иконку (`use`, `replenish`, `view`, `delete`, `edit`, `create`, `copy`, `copy-link`, `kick`, `revive` и др.), а доменное действие передаёт компонент через `icon` или разметку через slot `icon`. Для окраски доступны `accent`, `warning`, `success`, `info`, `danger` в prop `tone`; пункты без иконки не допускаются. |
