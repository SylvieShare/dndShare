...
| Плитка/карточка контента | `shared/ui/BaseTile.vue` | Общая поверхность с фоном, скруглением и тихой рамкой. Поддерживает цветовой акцент, полосу, диагональную `markColor`-засечку, tint, framed- и interactive-состояния; не создавать локальный базовый класс плитки. |
| Popover/контекстное меню | `shared/ui/BasePopover.vue`, `RowActionMenu.vue`, `RowActionItem.vue` | Неблокирующий контент, привязанный к управляющему элементу. `RowActionMenu` поддерживает стандартный trigger с многоточием и feature-trigger через slot, `block` растягивает строку-trigger, а `disabled` подавляет открытие во время drag. Меню остаётся внутри visual viewport, при нехватке места открывается вверх и ограничивает высоту внутренним скроллом. Появление и исчезновение анимируются относительно точки trigger, а `RowActionItem` даёт короткий press-feedback пункту и иконке; `prefers-reduced-motion` отключает движение. Каждый кликабельный пункт строится через `RowActionItem`: `action` выбирает стандартную иконку (`use`, `replenish`, `view`, `delete`, `edit`, `create`, `copy`, `copy-link`, `kick`, `revive` и др.), а доменное действие передаёт компонент через `icon` или разметку через slot `icon`. Для окраски доступны `accent`, `warning`, `success`, `info`, `danger` в prop `tone`; пункты без иконки не допускаются. |
