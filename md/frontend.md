...
На уровнях сценариев и блоков завершение link-жеста открывает общий редактор
необязательной подписи (до 240 символов). Клик по линии или уже показанной
подписи открывает `BasePopover` с изменением подписи и удалением перехода;
create/update/delete сразу синхронизируют соответствующий edge в серверном
graph state через `POST`, `PATCH` и `DELETE` routes.
...
холста сверху справа, перед правой панелью инструментов. Каждый action —
