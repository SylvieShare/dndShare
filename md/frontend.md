...
| Составное окно с шапкой | `shared/ui/AppEditorModal.vue` | Единая оболочка формы, просмотра или полноэкранного picker поверх `AppModal`: одинаковые фон и рамка, закреплённые заголовок/крестик/мобильный handle, отдельная прокрутка тела и опциональный закреплённый footer. |
...
- сложная предметная форма — feature-компонент, но его оболочка всегда
  `AppEditorModal`, если окну нужны заголовок и прокручиваемое содержимое;
