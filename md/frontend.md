...
| Морф-переход из элемента | `shared/ui/MorphSheet.vue` | Только когда окно должно анимированно раскрываться из конкретной плитки/кнопки. На desktop фон размывается, на mobile полноэкранный sheet оставляет фон без blur. Обычный диалог строится на `AppModal`. |
