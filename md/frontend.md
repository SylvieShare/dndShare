...
`max-height` и после загрузки использует фактическое соотношение файла: поэтому
новый кадр показывается целиком, а ещё не перегенерированные 3:1-обложки не
обрезаются в переходный период.
