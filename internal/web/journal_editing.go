...
func (s *Server) handleReorderJournalEntries(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(r, &req); err != nil || req.EntryIDs == nil {
		badRequest(w, "Укажите порядок событий")
		return
	}
	if err := s.store.ReorderJournalEntries(r.Context(), journal.ID, sectionID, req.EntryIDs); err != nil {
		writeJournalError(w, err)
		return
	}
	s.writeReloadedJournal(w, r, journal.UUID, http.StatusOK)
