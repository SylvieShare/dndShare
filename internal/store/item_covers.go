...
const retireItemCoverImageSQL = `
	UPDATE dndshare.storage_image img
	SET deleted = true, "type" = 'retired_item_cover'
	WHERE img.id = $1
...
// UpsertSystemItemCover registers a stable S3 object and assigns it to matching
// built-in items. The cover is intentionally independent from both icon formats.
func (s *Store) UpsertSystemItemCover(ctx context.Context, key, url, fileName, mimeType string, fileSize, typeID int64, aliases []string) (int64, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		...
		_, err = tx.Exec(ctx, `
			...
			SET url = $2, "type" = 'item_cover', deleted = false, file_name = $3, mime_type = $4, file_size = $5
	...
	rows, err := tx.Query(ctx, systemItemCoverItemsSQL, aliases, typeID)
	if err != nil {
		return 0, err
	}
	itemIDs := []int64{}
	...
	if len(itemIDs) > 0 {
		if _, err := tx.Exec(ctx, `UPDATE dndshare.item SET cover_image_id = $1 WHERE id = ANY($2::bigint[])`, imageID, itemIDs); err != nil {
			return 0, err
		}
	}
	for oldImageID := range oldImageIDs {
		if _, err := tx.Exec(ctx, retireItemCoverImageSQL, oldImageID); err != nil {
			return 0, err
		}
	}
...
func (s *Store) ListRetiredItemCoverKeys(ctx context.Context) ([]string, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT DISTINCT "key"
		FROM dndshare.storage_image
		WHERE "type" = 'retired_item_cover' AND deleted = true AND "key" IS NOT NULL
		ORDER BY "key"`)
	if err != nil {
		return nil, err
	}
