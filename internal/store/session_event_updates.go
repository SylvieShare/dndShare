...
func (s *Store) SessionMutableEventUpdates(ctx context.Context, sessionID, userID, afterID int64) ([]SessionEvent, error) {
	...
	rows, err := s.pool.Query(ctx, sessionEventSelect+sessionEventReadAccess+` AND e.session_id=$1 AND e.id<=$3 AND (e.event_type IN ('item_transfer','rps_challenge') OR e.data ? 'savingThrow' OR e.data ? 'damageRoll' OR e.data ? 'impacts') ORDER BY e.id DESC LIMIT 200`, sessionID, userID, afterID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
