...
-- A clean database must not depend on the obsolete condition suggest rows.
WITH standard(code, name, thesis) AS (
  VALUES
    ('blinded', 'Слепота', E'- Не видит и проваливает проверки зрения\n- Его атаки с помехой; атаки по нему с преимуществом'),
    ('charmed', 'Обворожение', E'- Не атакует и не вредит очаровавшему\n- Очаровавший имеет преимущество в социальных проверках'),
    ('deafened', 'Глухота', E'- Не слышит\n- Проваливает проверки слуха'),
    ('frightened', 'Испуг', E'- Помеха к проверкам характеристик и броскам атаки\n- Не может переместиться ближе к источнику испуга'),
    ('grappled', 'Захват', E'- Скорость 0 без бонусов\n- Заканчивается при недееспособности захватившего или разрыве дистанции'),
    ('incapacitated', 'Выход из строя', E'- Не может совершать действия и реакции'),
    ('invisible', 'Невидимость', E'- Нельзя увидеть без магии или особого чувства\n- Его атаки с преимуществом; атаки по нему с помехой'),
    ('paralyzed', 'Паралич', E'- Недееспособен, не двигается и не говорит\n- Проваливает спасброски Силы и Ловкости\n- Атаки по нему с преимуществом; попадание в 5 футах критическое'),
    ('petrified', 'Окменение', E'- Недееспособен, неподвижен и не замечает окружение\n- Сопротивление всему урону\n- Проваливает спасброски Силы и Ловкости; атаки по нему с преимуществом'),
    ('poisoned', 'Отравление', E'- Помеха к проверкам характеристик и броскам атаки'),
    ('prone', 'Расплатстанность', E'- Только ползает или встаёт за половину скорости\n- Его атаки с помехой\n- Атаки в 5 футах с преимуществом, дальше — с помехой'),
    ('restrained', 'Обездвиженность', E'- Скорость 0\n- Его атаки с помехой; атаки по нему с преимуществом\n- Помеха к спасброскам Ловкости'),
    ('stunned', 'Шок', E'- Недееспособен, не двигается и говорит сбивчиво\n- Проваливает спасброски Силы и Ловкости\n- Атаки по нему с преимуществом'),
    ('unconscious', 'Без сознания', E'- Недееспособен, падает и выпускает предметы\n- Проваливает спасброски Силы и Ловкости\n- Атаки по нему с преимуществом; попадание в 5 футах критическое')
)
INSERT INTO dndshare.item (name, data, type_id)
SELECT standard.name,
       jsonb_build_object(
         'code', standard.code,
         'thesis', standard.thesis,
         'desc', standard.thesis,
         'polarity', 'negative',
         'color', '#e0524e',
         'stacking', 'single',
...
WHERE NOT EXISTS (
  ...
  WHERE effect.type_id = 15
    ...
    AND (effect.data ->> 'code' = standard.code OR lower(effect.name) = lower(standard.name))
);

WITH canonical(code, name, thesis) AS (
  VALUES
    ('blinded', 'Слепота', E'- Не видит и проваливает проверки зрения\n- Его атаки с помехой; атаки по нему с преимуществом'),
    ('charmed', 'Обворожение', E'- Не атакует и не вредит очаровавшему\n- Очаровавший имеет преимущество в социальных проверках'),
    ('deafened', 'Глухота', E'- Не слышит\n- Проваливает проверки слуха'),
    ...
    ('shield_of_faith', 'Щит веры', E'- +2 к КД\n- Требует концентрации'),
    ('exhaustion', 'Истощение', E'- Уровни накапливают штрафы\n- Продолжительный отдых снимает один уровень'),
    ('inspiration', 'Вдохновение', E'- Преимущество на один бросок атаки, проверки или спасброска')
)
UPDATE dndshare.item effect
SET data = jsonb_set(
  jsonb_set(effect.data, '{code}', to_jsonb(canonical.code), true),
  '{thesis}',
  to_jsonb(canonical.thesis),
  true
)
FROM canonical
WHERE effect.type_id = 15
  AND effect.user_id IS NULL
  AND (effect.data ->> 'code' = canonical.code OR lower(effect.name) = lower(canonical.name));
