...
-- Правила игрока используют короткий переиспользуемый словарь стандартных
-- действий. Статьи, примеры и версионные исключения остаются frontend-контентом;
-- suggest хранит только устойчивый code, подпись и краткую справку.
INSERT INTO dndshare.suggest_type (id, name, source_id, color, count_items)
VALUES (24, 'Действия в бою', (SELECT id FROM dndshare."source" WHERE lower(name) = 'dnd5e' LIMIT 1), '#e85c8a', 11)
ON CONFLICT (id) DO UPDATE SET
    name = EXCLUDED.name,
    source_id = EXCLUDED.source_id,
    color = EXCLUDED.color;

WITH action_seed(code, value, description) AS (
    VALUES
        ('attack',       'Атака',                'Совершите одну рукопашную или дальнобойную атаку. Некоторые особенности позволяют сделать несколько атак этим действием.'),
        ('cast-spell',   'Наложение заклинания', 'Примените заклинание со временем накладывания «1 действие». Заклинания также могут использовать бонусное действие, реакцию или больше времени.'),
        ('dash',         'Рывок',                 'Получите дополнительное перемещение на этот ход, равное вашей скорости после применения модификаторов.'),
        ('disengage',    'Отход',                 'Ваше перемещение не провоцирует атаки до конца текущего хода.'),
        ('dodge',        'Уклонение',             'До начала следующего хода атаки видимых вами существ совершаются с помехой, а спасброски Ловкости — с преимуществом. Эффект теряется при недееспособности или скорости 0.'),
        ('help',         'Помощь',                'Дайте союзнику преимущество в подходящей проверке или помогите с атакой по существу рядом с вами.'),
        ('hide',         'Засада',                'Попытайтесь спрятаться: мастер определяет, возможно ли это, и обычно просит проверку Ловкости (Скрытность).'),
        ('ready',        'Подготовка',            'Выберите заметный триггер и действие в ответ. При срабатывании потратьте реакцию или откажитесь от подготовленного действия.'),
        ('search',       'Поиск',                 'Сосредоточьтесь на поиске и совершите назначенную мастером проверку Внимательности или Анализа.'),
        ('use-object',   'Использование предмета','Используйте предмет, которому требуется действие. Одно простое взаимодействие с предметом обычно доступно в составе хода.'),
        ('improvise',    'Импровизированное действие', 'Опишите намерение, которого нет в списке. Мастер решит, возможно ли оно и какой бросок или цена нужны.')
)
INSERT INTO dndshare.suggest (type_id, value, code, "desc")
SELECT 24, seed.value, seed.code, seed.description
FROM action_seed seed
WHERE NOT EXISTS (
    SELECT 1 FROM dndshare.suggest current
    WHERE current.type_id = 24 AND current.user_id IS NULL AND current.code = seed.code
);
...
UPDATE dndshare.suggest current
SET value = seed.value,
    "desc" = seed.description
FROM action_seed seed
WHERE current.type_id = 24
  AND current.user_id IS NULL
  AND current.code = seed.code;

UPDATE dndshare.suggest_type
SET count_items = (SELECT COUNT(*) FROM dndshare.suggest WHERE type_id = 24)
WHERE id = 24;
