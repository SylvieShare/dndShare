...
func TestSessionPresentationSchemaKeepsMaterialsAndOneLiveState(t *testing.T) {
	for _, fragment := range []string{
		...
		"ADD COLUMN IF NOT EXISTS health_display",
		"ADD COLUMN IF NOT EXISTS show_graveyard",
		"ADD COLUMN IF NOT EXISTS display_scale",
		"health_display IN ('numbers', 'words')",
		"display_scale BETWEEN 75 AND 125",
		"DROP COLUMN IF EXISTS show_initiative",
