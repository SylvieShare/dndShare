...
const markStorageImageDeletedIfUnreferencedSQL = `UPDATE dndshare.storage_image img
		  ...
		  WHERE img.id = $1
		    ...
		    AND NOT EXISTS (SELECT 1 FROM dndshare.battle_map map WHERE map.asset_id = img.id)
		    AND NOT EXISTS (SELECT 1 FROM dndshare.session_map map WHERE map.asset_id = img.id)
